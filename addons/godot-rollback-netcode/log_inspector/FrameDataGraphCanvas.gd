...
const _SyncLogger = preload("res://addons/godot-rollback-netcode/Logger.gd")
...
const FRAME_TYPE_COLOR = {
	_SyncLogger.FrameType.INTERFRAME: Color(0.3, 0.3, 0.3),
	_SyncLogger.FrameType.TICK: Color(0.0, 0.75, 0.0),
	_SyncLogger.FrameType.INTERPOLATION_FRAME: Color(0.0, 0.0, 0.5),
}
...
func _draw_peer(peer_id: int, peer_rect: Rect2, draw_data: Dictionary) -> void:
	...
	while frame.start_time <= absolute_end_time:
		...
		if frame_rect.intersects(extended_peer_rect):
			...
			if fatal_error:
				...
				if frame.data.has('skip_reason'):
					...
					match int(frame.data['skip_reason']):
						_SyncLogger.SkipReason.INPUT_BUFFER_UNDERRUN:
							tick_letter = 'B'
						_SyncLogger.SkipReason.WAITING_TO_REGAIN_SYNC:
							tick_letter = 'W'
						_SyncLogger.SkipReason.ADVANTAGE_ADJUSTMENT:
							tick_letter = 'A'
					if tick_letter != '':
			...
			if frame.type == _SyncLogger.FrameType.TICK and frame.data.has('tick') and not skipped:
