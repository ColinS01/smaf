...
func replay_to_current_frame() -> void:
	...
	for frame_id in range(replay_frame, log_data.frames[replay_peer_id].size()):
		...
		if frame_data.type == _SyncLogger.FrameType.TICK and frame_data.data.get('skipped', false):
...
func _send_replay_frame_data(frame_data: LogData.FrameData) -> void:
	...
	if frame_type == _SyncLogger.FrameType.TICK:
	...
	elif frame_type == _SyncLogger.FrameType.INTERPOLATION_FRAME:
