	...
	if not _mechanized:
		...
		if _ticks_spent_regaining_sync > 0:
			...
			if not _cleanup_buffers():
				...
				if _logger:
					_logger.skip_tick(_SyncLogger.SkipReason.INPUT_BUFFER_UNDERRUN, start_time)
				return
			...
			if min_lag_to_regain_sync > 0 and _calculate_max_local_lag() > min_lag_to_regain_sync:
				...
				if not _spectating:
					_send_input_messages_to_all_peers()
				if _logger:
					_logger.skip_tick(_SyncLogger.SkipReason.WAITING_TO_REGAIN_SYNC, start_time)
				return
		...
		if _skip_ticks > 0:
			...
			if _skip_ticks == 0:
				...
				if not _spectating:
					_send_input_messages_to_all_peers()
				if _logger:
					_logger.skip_tick(_SyncLogger.SkipReason.ADVANTAGE_ADJUSTMENT, start_time)
				return
